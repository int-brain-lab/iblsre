...
async def delete_cancelled_runs(max_duration_seconds=20):
    async with prefect.get_client() as client:
        ...
        flow_runs = await client.read_flow_runs(flow_run_filter=FlowRunFilter(state=dict(type=dict(any_=["CANCELLED"]))))
...
async def delete_short_runs(max_duration_seconds=20):
    async with prefect.get_client() as client:
        # Fetch completed flow runs
        flow_runs = await client.read_flow_runs(flow_run_filter=FlowRunFilter(state=dict(type=dict(any_=["COMPLETED"]))))
        for run in flow_runs:
...
def run_job_creator_task(one=None, data_repository_name=None, root_path=subjects_path):
    ...
    if not any(tasks):
        ...
        task_dict = {
            ...
            "cpu": t.cpu,
            "ram": t.ram,
            "module": "deploy.serverpc.crontab.report_create_jobs",
            "parents": [],
            "level": t.level,
            "status": "Empty",
            "name": t.name,
    ...
    status = t.run()
    patch_data = {
        "time_elapsed_secs": t.time_elapsed_secs,
        "log": t.log,
        "version": t.version,
...
def _get_jobs(mode, env=(None,), max_tasks=MAX_TASKS):
    ...
    for tdict in waiting_tasks:
        ...
        if last_session != tdict["session"]:
            ...
            session_path = Path(subjects_path).joinpath(ses["subject"], ses["start_time"][:10], str(ses["number"]).zfill(3))
...
if __name__ == "__main__":
    ...
    prefect.deploy(
        create_jobs.to_deployment(
            ...
            concurrency_limit=ConcurrencyLimitConfig(limit=1, collision_strategy=ConcurrencyLimitStrategy.CANCEL_NEW),
